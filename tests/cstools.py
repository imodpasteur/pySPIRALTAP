...
def reconstruct_1Dspiral(measure, basis, maxiter=500, verbose=0, W=[], penalty='canonical'):
...
def reconstruct_parallel2d(img, basis, ncores=8, algo=reconstruct_1Dmagic):
    ...
    tmp=Parallel(n_jobs=ncores)(delayed(_reconstruct_parallel)(img[i,:], basis, algo=algo) for i in range(N))
    for (i, j) in enumerate(tmp):
        rr = j
        out[i,:]=rr.flatten()
    return out

def _reconstruct_parallel(m, bf, poisson=False, gaussian=0, algo=None):
    if algo==None:
